...
instead of a complete traversal of the game tree. I keep Kuhn Poker as a
sanity check, but use Leduc Poker as the main experiment because it is
large enough to show the computational motivation for sampling. I
implement full-tree CFR and external-sampling MCCFR in Python, compare
them on Leduc Poker, and use exact best responses to measure
exploitability. The main message is that MCCFR keeps the same local
regret-minimization structure as CFR, but replaces exact counterfactual
values with sampled estimates.
\end{abstract}
...
The main experiment compares full-tree CFR and external-sampling MCCFR on
Leduc Poker. I ran full-tree CFR for $500$ iterations and MCCFR for
$100{,}000$ iterations with three random seeds. The MCCFR numbers below
...
Both methods reduce exploitability substantially from the uniform-strategy
baseline. CFR has a smoother curve because every update is exact for the
current profile. MCCFR is noisier, especially at low iteration counts,
because each update depends on sampled chance and opponent actions.
...
iteration, while external sampling visits about $42.6$ nodes per
iteration on average. Thus MCCFR can take many more iterations for a
similar amount of tree traversal. In the table, MCCFR uses $200$ times as
many iterations as CFR, but still performs fewer total regret-update node
visits.

\begin{figure}[H]
\centering
\includegraphics[width=0.82\linewidth]{figures/leduc_cfr_vs_external_sampling_mccfr.png}
\caption{Exploitability of full-tree CFR and external-sampling MCCFR on Leduc Poker.}
\end{figure}
...
outcome sampling, larger Leduc variants, and abstraction methods such as
bucketing.
